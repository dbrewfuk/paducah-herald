...
@section('title', ($episode->title ?? 'Herald Insider') . ' | The Paducah Herald')
@section('body-class', 'insider-page')

@section('no-adstrip', true)
...
<nav class="topnav topnav-dark" aria-label="Section navigation">
  <div class="topnav-inner">
    <a class="topnav-link" href="{{ route('weekly-edition.index') }}">E-Edition</a>
    <a class="topnav-link" href="{{ route('world-in-brief') }}">In Brief</a>
    @php $navSections = \App\Models\Section::orderBy('position')->get(); @endphp
    @foreach($navSections as $section)
      <a class="topnav-link" href="{{ route('sections.show', $section->slug) }}">{{ $section->title }}</a>
    @endforeach
    <a class="topnav-link is-active" href="{{ route('insider.index') }}">Insider</a>
  </div>
</nav>
@endsection
...
@if(isset($episode))
  <div class="episode-hero">
    <div>
      ...
      @if($episode->sponsor_name)
        <div class="episode-sponsor">
          <span class="episode-sponsor-label">Sponsored by</span>
          @if($episode->imageObject('sponsor_logo'))
            <img src="{{ $episode->image('sponsor_logo', 'default') }}" alt="{{ $episode->sponsor_name }}" class="episode-sponsor-img" style="height:32px;opacity:0.75" />
          @else
            <span style="font-family:var(--sans);font-size:14px;color:rgba(255,255,255,0.6)">{{ $episode->sponsor_name }}</span>
          @endif
        </div>
      @endif
    </div>
    <div>
      <div class="episode-video-wrap">
        @if($episode->imageObject('thumbnail'))
          <img class="episode-video-thumb" src="{{ $episode->image('thumbnail', 'default') }}" alt="{{ $episode->title }}" />
        @endif
        @if($episode->video_url)
          <a href="{{ $episode->video_url }}" class="episode-play-overlay" target="_blank" rel="noopener">
        @else
        ...
        @if($episode->video_url) </a> @else </div> @endif
      </div>
      @if($episode->publish_start_date)
        <p style="font-size:12px;font-family:var(--sans);color:rgba(255,255,255,0.4);margin-top:10px">
          Recorded {{ \Carbon\Carbon::parse($episode->publish_start_date)->format('F j Y') }}
        </p>
      @endif
    </div>
  </div>
  ...
  <div class="episode-body">
    <main>
      @if($episode->episode_summary)
        ...
        <div class="ep-summary-text">{{ $episode->episode_summary }}</div>
      @endif
    </main>
    <aside class="ep-sidebar">
      <p class="ep-nav-label">More episodes</p>
      @foreach($otherEpisodes as $ep)
        <a class="ep-nav-item" href="{{ route('insider.show', $ep->slug) }}">
          <p class="ep-nav-tag">
            {{ $ep->publish_start_date ? \Carbon\Carbon::parse($ep->publish_start_date)->format('M j, Y') : 'Insider' }}
          </p>
          <p class="ep-nav-title">{{ $ep->title }}</p>
        </a>
      @endforeach
    </aside>
  </div>
...
@else
  ...
  <div class="insider-grid">
    @forelse($episodes as $ep)
      <a class="insider-card" href="{{ route('insider.show', $ep->slug) }}">
        @if($ep->imageObject('thumbnail'))
          <img class="insider-card-img" src="{{ $ep->image('thumbnail', 'default') }}" alt="{{ $ep->title }}" />
        @else
          <div class="insider-card-placeholder"></div>
        @endif
        @if($ep->publish_start_date)
          <p class="insider-card-date">{{ \Carbon\Carbon::parse($ep->publish_start_date)->format('F j, Y') }}</p>
        @endif
        <p class="insider-card-title">{{ $ep->title }}</p>
        @if($ep->sponsor_name)
          <p class="insider-card-sponsor">Sponsored by {{ $ep->sponsor_name }}</p>
        @endif
      </a>
    @empty
